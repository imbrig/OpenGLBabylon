...
public:
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    const float ratio = static_cast<float>(getEngine()->getRenderWidth()) / static_cast<float>(getEngine()->getRenderHeight());
    BoxOptions options;
    options.size = 5.f;
    ...
    shaderMaterialOptions.uniforms = {"iTime", "worldViewProjection", "iAspectRatio", "iResolution"};
    ...
    scene->onAfterCameraRenderObservable.add([this](Camera*, EventState&) {
      const Vector2 resolution{static_cast<float>(_engine->getRenderWidth()), static_cast<float>(_engine->getRenderHeight())};
...
std::shared_ptr<IRenderableScene> MakeShaderMaterialDoubleHelixScene(ICanvas* canvas)
{
  return std::make_shared<ShaderMaterialDoubleHelixScene>(canvas);
}
